...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function admin_enqueue() {
			...
			wp_register_script('simple-image-windget-js', $this->url . 'assets/js/simple-image-widget.js', array('jquery'), time(), true);
			...
			wp_enqueue_script('simple-image-windget-js');
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				//template wrapper
				echo '<div id="engine-template-wrapper-'.$template_id.'" class="row">';
		...
		function contextual_help() {
			...
			if($screen->id == $this->page) {
				...
				$screen->set_help_sidebar(
					'<p><strong>' . __('For more information:', 'engine') . '</strong></p>' .
					'<p>' . __('<a href="http://aquagraphite.com/api/documentation/aqua-page-builder" target="_blank">Documentation</a>'. 'engine') . '</p>' .
					'<p>' . __('<a href="http://aquagraphite.com/api/changelog/aqua-page-builder" target="_blank">Changelog</a>', 'engine') . '</p>'
				);
				
				// Main overview tab
				$screen->add_help_tab( array(
				'id'		=> 'overview',
				'title'		=> __('Overview'. 'engine'),
